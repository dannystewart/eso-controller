...
function Azurah:ConfigureActionBarElements()
	...
	-- special case for hiding companion ultimate button text
	...
	if (IsInGamepadPreferredMode()) then -- special case to handle changes to ultimate button in Gamepad mode
		_G['ActionButton8LeftKeybind']:SetAlpha(db.hideBindText and 0 or 1)
        _G['ActionButton8LeftKeybind']:SetHidden(db.hideBindText)

        _G['ActionButton8RightKeybind']:SetAlpha(db.hideBindText and 0 or 1)
        _G['ActionButton8RightKeybind']:SetHidden(db.hideBindText)
