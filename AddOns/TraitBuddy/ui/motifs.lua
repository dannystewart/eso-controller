local function MotifHeaderSetup(node, control, data, open)
	...
	if linkName == nil then linkName = zo_strformat(SI_CRAFTING_UNKNOWN_NAME) end
...
function TB_Motifs:AddFilters()
	...
	local text = zo_strformat(SI_ALCHEMY_UNKNOWN_RESULT, motifText)
	if text:sub(text:len())=="." then
		text = text:sub(0, text:len()-1)
	end
	local texts = {
		[1] = GetString(SI_GAMEPAD_GUILD_HISTORY_SUBCATEGORY_ALL),
		[2] = zo_strformat(TB_MOTIF_ALMOST),
		[3] = text,
		[4] = zo_strformat(SI_ITEM_FORMAT_STR_KNOWN_ITEM_TYPE, motifText)
	}
		...
		if (children) then
			...
			for i = 1, #children do
				...
				if children[i]:IsLeaf() then
					...
					ctrl.knowText:SetText(known.."/"..self.numChapters)
					ctrl.no:SetColor(colours.not_known.r, colours.not_known.g, colours.not_known.b)
...
function TB_Motifs:UpdateTotal()
	self.total:SetText(zo_strformat("<<1>>/<<2>> (<<3>>/<<4>>)", self.total.motifs, self.numMotifsTotal, self.total.chapters, self.numChaptersTotal))
end

function TB_Motifs:Total_OnMouseEnter(control)
	local r, g, b = ZO_NORMAL_TEXT:UnpackRGB()
	InitializeTooltip(InformationTooltip, control, TOPLEFT, 0, 5, BOTTOMLEFT)
	InformationTooltip:AddLine(zo_strformat("<<1>>: <<2>> / <<3>>", GetString(SI_SPECIALIZEDITEMTYPE60), control.motifs, self.numMotifsTotal), "ZoFontGame", r,g,b, LEFT, MODIFY_TEXT_TYPE_NONE, TEXT_ALIGN_CENTER, true)
	InformationTooltip:AddLine(zo_strformat("<<1>>: <<2>> / <<3>>", GetString(SI_SPECIALIZEDITEMTYPE61), control.chapters, self.numChaptersTotal), "ZoFontGame", r,g,b, LEFT, MODIFY_TEXT_TYPE_NONE, TEXT_ALIGN_CENTER, true)
end
