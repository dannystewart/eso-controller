...
function TB_InventoryInsight:DisplayTooltip(control, GamePadMode, matchCraftingSkillType, matchResearchLineIndex, matchTraitIndex)
	...
	if self:Installed() then
		...
		for itemLink,details in pairs(IIfA.database) do
		...
		for k,v in pairs(onCharacter.names) do
			if names:len()==0 then
				names = v.count.."x "..zo_strformat("<<1>>",v.name)
			else
				names = names..", "..v.count.."x "..zo_strformat("<<1>>",v.name)
			end
		end
		if GamePadMode then
			local style2 = TraitBuddy:GetGamepadStyle(2)
			local summary = zo_iconTextFormatNoSpace("esoui/art/MainMenu/menuBar_character_up.dds", 42, 42, onCharacter.count) .." ".. zo_iconTextFormatNoSpace("esoui/art/icons/servicemappins/servicepin_bank.dds", 42, 42, inBank) .. zo_iconTextFormatNoSpace("esoui/art/MainMenu/menuBar_guilds_up.dds", 42, 42, inGuild.count)
			...
			for k,v in pairs(inGuild.names) do
				control:AddLine(v.count.."x "..zo_strformat("<<1>>",v.name), style2, control:GetStyle("bodySection"))
			end
		else
			local r,g,b = ZO_TOOLTIP_DEFAULT_COLOR:UnpackRGB()
			local summary = zo_iconTextFormatNoSpace("esoui/art/MainMenu/menuBar_character_up.dds", 32, 32, onCharacter.count) .." ".. zo_iconTextFormatNoSpace("esoui/art/icons/servicemappins/servicepin_bank.dds", 32, 32, inBank) .. zo_iconTextFormatNoSpace("esoui/art/MainMenu/menuBar_guilds_up.dds", 32, 32, inGuild.count)
			...
			for k,v in pairs(inGuild.names) do
				control:AddLine(v.count.."x "..zo_strformat("<<1>>",v.name), "TBFontGame16", r,g,b, LEFT, MODIFY_TEXT_TYPE_NONE, TEXT_ALIGN_CENTER, true)
			end
		end
	end
end