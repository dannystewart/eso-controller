...
function TB_UpdateLater:ResearchCompleted(craftingSkillType, researchLineIndex, traitIndex)
	local id = "c"..craftingSkillType.."r"..researchLineIndex.."t"..traitIndex
...
function TB_UpdateLater:CheckCompletedEarlierNew()
	local c = TraitBuddy:GetCharacter(TraitBuddy.characterId)
	if c then
		if c.completed then
			d(zo_strformat("TraitBuddy DEBUG: CheckCompletedEarlier num completed <<1>>", NonContiguousCount(c.completed)))
			for k,v in pairs(c.completed) do
				d(zo_strformat("TraitBuddy DEBUG: key <<1>>", k))
				if v.done then
					--c.completed[k] = nil
				else
					self:ResearchCompleted(v.craftingSkillType, v.researchLineIndex, v.traitIndex)
			...
			--if NonContiguousCount(c.completed) == 0 then
			--	c.completed = nil
			--end
		end
	end
