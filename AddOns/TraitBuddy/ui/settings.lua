...
function TB_Settings:CreatePanel()
	...
	local panelData = {
		...
		displayName = "|cff8800"..TraitBuddy.ADDON_NAME.."|r",
		author = "Weolo",
	...
	local optionsData = {
		{
			...
			type = "header",
			name = "|c3f7fff"..GetString(TB_OP_HEADING6).."|r"
		},{
			type = "checkbox",
			name = TB_OP_BAG,
...
function TB_Settings_OnMouseEnter(control, tooltip)
...
function TB_Settings:Character_Delete_OnClicked(control)
    ...
    dialog.mainText.text = zo_strformat(TB_OP_DELETE_TT, TraitBuddy.ADDON_NAME, TraitBuddy:GetCharacter(id).name).."?"
...
function TB_Settings:AddCharacter(id, last)
	...
	delButton.data.tooltipText = zo_strformat(TB_OP_DELETE_TT, TraitBuddy.ADDON_NAME, c.name)
	return control
end
